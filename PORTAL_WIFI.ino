...
dnsServer.start(DNS_PORT, "*", apIP);
  ...
  asyserver.on("/redirect", HTTP_GET, [](AsyncWebServerRequest *request) {
  Serial.println("/redirect requested");
  handlePortalRoot();    
  request->send(200, "text/html", toSend); //send the html code to the client
  });

  asyserver.on("/fwlink", HTTP_GET, [](AsyncWebServerRequest *request) {
  Serial.println("/redirect requested");
  handlePortalRoot();    
  request->send(200, "text/html", toSend); //send the html code to the client
  });

  asyserver.on("/Bback", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  asyserver.on("/wifiForm", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  asyserver.on("/PORTAL_STYLE", HTTP_GET, [](AsyncWebServerRequest *request) {
...
//                   CONNECTING TO WIFI
// **********************************************************
  asyserver.on("/wifiCon", HTTP_GET, [](AsyncWebServerRequest *request) {
...
//  server.onNotFound(handlePortalNotFound);
  asyserver.onNotFound([](AsyncWebServerRequest *request) {
...
//  server.on("/STATIC_ERASE", resetStatic);   
  asyserver.on("/STATIC_ERASE", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  asyserver.on("/close", HTTP_GET, [](AsyncWebServerRequest *request) {
  handlePortalClose();
  request->send(200, "text/html", toSend); //send the html code to the client 
   }); 
