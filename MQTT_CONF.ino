...
<div id='msect'><center><div id='bo'></div>
<ul>
<li id='fright'><span class='close' onclick='cl();'>&times;</span>
<li><a href='/MQTT_TEST'>test</a></ul>
<li id='sub'><a href='#' onclick='submitFunction("/")'>save</a></li></ul></div>
...
<!--    <option value='3' fm_3>format 3</option>
  ...
  <tr><td>state idx:&nbsp<td><input class='inp2' name='mqtidx' value='{idx}' size='4' length='4'></tr>
...
webPage.replace("{idx}"         , String(Mqtt_idx) ); 
...
void handleMQTTconfig(AsyncWebServerRequest *request) {
  ...
  strcpy( Mqtt_Port    , request->getParam("mqtPort")    ->value().c_str() );
   Mqtt_idx =            request->arg("mqtidx").toInt(); //values are 0 1 2
...
//  Mqtt_stateIDX = request->arg("mqidx").toInt(); //values are 0 1 2
  ...
  mqttConfigsave();  // 
  actionFlag=24; // reconnect mqtt with these new settings
